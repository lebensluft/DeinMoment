...
@media(max-width:760px){
  ...
  .option-grid,.option-grid.three,.option-grid.goal-grid,.info-grid{
    grid-template-columns:1fr;
  }
  .card{padding:18px;}
            ...
            <div class="figure-stage">
              ...
              <div class="breath-person" id="breathPerson">
                <svg viewBox="0 0 140 180" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                  ...
                  <path d="M35 72 C48 62, 56 58, 70 58 C84 58, 92 62, 105 72" stroke="#173d33" stroke-width="4" stroke-linecap="round"/>
...
function filterForPreferences(list){
  ...
  if(cp==="low"){
    filtered=filtered.filter(t=>t.style!=="sigh" || goal==="presentation");
    filtered=filtered.filter(t=>!["march"].includes(t.style));
  }
...
function startBreathingAnimation(sequence){
  ...
  const starter=setTimeout(()=>runPhase(sequence,0),250);
